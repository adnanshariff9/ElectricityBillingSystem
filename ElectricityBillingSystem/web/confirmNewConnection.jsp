...
<body>
    ...
    <main>
        <center>
           ...
           <div class="container">
    <h2>Confirm details for New Connection</h2>
    <!-- Add a form for confirmation and update -->
    <form action="NewConnectionUsers" method="post">
    ...
    <select id="newConnectionStatus" name="newConnectionStatus" required>
        <option value="not applied" <% if ("not applied".equals(session.getAttribute("newConnectionStatus"))) { %>selected<% } %>>Not Applied</option>
        <option value="applied" <% if ("applied".equals(session.getAttribute("newConnectionStatus"))) { %>selected<% } %>>Apply</option>
    </select>
</div>
<br/>
